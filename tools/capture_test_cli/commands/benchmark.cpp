 ...
 * @brief 运行单个后端基准测试
 */
BackendResult RunSingleBenchmark(BackendType        type,
                                 ...
                                 bool               verbose)
{
    ...
    if (!init_result) {
        result.error_message =
            fmt::format("Init failed: {}", static_cast<int>(init_result.error()));
        return result;
    }
    ...
    if (!start_result) {
        result.error_message =
            fmt::format("Start failed: {}", static_cast<int>(start_result.error()));
 ...
 * @brief 保存结果为 JSON
 */
void SaveResultsToJson(const std::vector<BackendResult>& results, const std::string& filename)
{
    nlohmann::json j;
...
{
    fmt::print(fg(fmt::color::cyan), "=== AAM Capture Backend Benchmark ===\n\n");
    fmt::print("Target: {}\n", config.target_id);
    ...
    for (auto backend_type : config.backends) {
        ...
        auto result =
            RunSingleBenchmark(backend_type, config.target_id, config.duration_ms, config.verbose);
