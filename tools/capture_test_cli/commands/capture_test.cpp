...
namespace {
    ...
    l0::PixelFormat ToL0PixelFormat(PixelFormatType type) {
        switch (type) {
            case PixelFormatType::RGBA: return l0::PixelFormat::RGBA32;
            case PixelFormatType::BGRA: return l0::PixelFormat::BGRA32;
            case PixelFormatType::RGB: return l0::PixelFormat::RGB24;
            case PixelFormatType::Gray: return l0::PixelFormat::Unknown;
        }
...
int ExecuteCaptureTest(const CaptureTestConfig& config) {
    ...
    while (true) {
        ...
        if (frame_saver && frame_count % config.save_interval == 0) {
            if (frame_saver->SaveFrame(metadata, pixel_data, 
                                       PixelFormatToString(config.format))) {
                saved_count++;
            }
        }
        ...
        if (config.show_preview) {
            ...
            cv::Mat frame(metadata.height, metadata.width, cv_type, 
            ...
            // 添加信息叠加
            std::string info = fmt::format("Frame: {} | FPS: {:.1f}", 
                                          frame_count, stats.GetCurrentFps());
            cv::putText(preview_frame, info, cv::Point(10, 30), 
        ...
        if (config.verbose && frame_count % 30 == 0) {
            fmt::print("Progress: {} ms / {} ms | Frames: {} | FPS: {:.1f}\n",
                      elapsed, config.duration_ms, frame_count, stats.GetCurrentFps());
        }
    }
