 ...
 * @brief 运行单个后端对比测试
 */
CompareResult RunCompareTest(BackendType        type,
...
{
    fmt::print(fg(fmt::color::cyan), "=== AAM Backend Comparison ===\n\n");
    fmt::print("Target: {}\n", config.target_id);
    ...
    for (auto backend_type : config.backends) {
        ...
        auto result = RunCompareTest(
            backend_type, config.target_id, config.duration_ms, config.save_frames, output_dir);
    ...
    if (results.size() >= 2 && config.save_frames) {
        ...
        for (size_t i = 0; i < results.size(); ++i) {
            for (size_t j = i + 1; j < results.size(); ++j) {
                if (results[i].success && results[j].success && !results[i].sample_frame.empty()
                    && !results[j].sample_frame.empty()) {
                    double similarity =
                        CalculateSimilarity(results[i].sample_frame, results[j].sample_frame);
                    fmt::print("{} vs {}: {:.1f}% similar\n",
                               results[i].backend_name,
                               results[j].backend_name,
                               similarity * 100);
                }
            }
        }
    ...
    if (best) {
        fmt::print(fg(fmt::color::green), "Recommended backend: {}\n", best->backend_name);
        fmt::print("  - Average FPS: {:.2f}\n", best->avg_fps);
        fmt::print("  - Consistency: {:.1f}%\n", best->consistency * 100);
        fmt::print("  - Total frames: {}\n", best->frames_captured);
    }
    else {
        fmt::print(fg(fmt::color::red), "No backend succeeded in the comparison.\n");
    }
