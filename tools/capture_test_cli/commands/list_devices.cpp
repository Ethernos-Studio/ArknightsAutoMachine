...
void ListMaaAdbDevices(bool verbose) {
    ...
    if (!device_list) {
        std::cout << fmt::format(fg(fmt::color::red), 
                                 "  Error: Failed to create device list\n");
        return;
    }
    
    if (!MaaToolkitAdbDeviceFind(device_list)) {
        std::cout << fmt::format(fg(fmt::color::gray), 
                                 "  No MAA ADB devices found\n");
    ...
    for (MaaSize i = 0; i < device_count; ++i) {
        ...
        if (verbose) {
            ...
            MaaAdbScreencapMethod screencap_methods = MaaToolkitAdbDeviceGetScreencapMethods(device);
...
void ListMaaDesktopWindows(bool verbose) {
    ...
    if (!window_list) {
        std::cout << fmt::format(fg(fmt::color::red), 
                                 "  Error: Failed to create window list\n");
        return;
    }
    
    if (!MaaToolkitDesktopWindowFindAll(window_list)) {
        std::cout << fmt::format(fg(fmt::color::gray), 
                                 "  No MAA desktop windows found\n");
    ...
    if (valid_count == 0) {
        std::cout << fmt::format(fg(fmt::color::gray), 
                                 "  No valid desktop windows found\n");
    }
...
int ExecuteListDevices(bool verbose) {
    std::cout << fmt::format(fg(fmt::color::cyan), 
                             "=== AAM Capture Backend Device List ===\n\n");
    ...
    if (adb_devices && !adb_devices->empty()) {
        for (const auto& device : *adb_devices) {
            ...
            if (verbose) {
                // 获取设备详细信息
                auto detail_result = AdbCaptureBackend::ExecuteAdbCommand(
                ...
                if (android_version) {
                    std::cout << fmt::format("    Android: {}\n", 
                                             android_version->empty() ? "Unknown" : *android_version);
                }
            }
        }
    } else {
    ...
    if (MaaToolkitConfigInitOption(user_path.c_str(), default_config.c_str())) {
        ...
        std::cout << fmt::format(fg(fmt::color::red), 
                                 "  Error: Failed to initialize MaaToolkit\n");
    }
#else
    std::cout << fmt::format(fg(fmt::color::yellow), "[MAA Framework Devices]\n");
    std::cout << fmt::format(fg(fmt::color::gray), 
                             "  MAA Framework support is not enabled\n");
    std::cout << fmt::format("  Build with -DAAM_MAAFW_ENABLED=ON to enable MAA support\n");
#endif
    
    std::cout << "\n";
    std::cout << fmt::format(fg(fmt::color::green), 
                             "=== End of Device List ===\n");
