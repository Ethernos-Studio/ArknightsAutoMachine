...
[[nodiscard]] ParsedArgs CliParser::Parse() const
{
    ...
    cxxopts::Options options(std::string(kAppName), "AAM Capture Backend Test CLI");

    // 全局选项
    options.add_options("Global")("h,help", "Show help message")("v,version",
                                                                 "Show version information")(
        "verbose", "Enable verbose output", cxxopts::value<bool>()->default_value("false"));

    // 子命令：list
    options.add_options("List")("l,list", "List available capture devices/windows");

    // 子命令：capture
    options.add_options("Capture")("c,capture", "Run capture test")(
        "b,backend",
        "Capture backend (adb|maa|win32|auto)",
        cxxopts::value<std::string>()->default_value("auto"))(
        "t,target", "Target device/window ID", cxxopts::value<std::string>())(
        "f,format",
        "Output pixel format (rgba|bgra|rgb|gray)",
        ...
        "o,output", "Output directory for saved frames", cxxopts::value<std::string>())(
        "save-frames",
        "Save captured frames to disk",
        cxxopts::value<bool>()->default_value("false"))(
        "save-interval", "Frame save interval", cxxopts::value<uint32_t>()->default_value("30"))(
        "preview", "Show preview window", cxxopts::value<bool>()->default_value("false"));

    // 子命令：benchmark
    options.add_options("Benchmark")("benchmark", "Run performance benchmark")(
        "backends",
        "Backends to benchmark (comma-separated: adb,maa,win32)",
        cxxopts::value<std::string>()->default_value("adb,maa,win32"))(
        "benchmark-duration",
        "Duration per backend in milliseconds",
        cxxopts::value<uint32_t>()->default_value("30000"))(
        "result-file", "Benchmark result output file", cxxopts::value<std::string>());

    // 子命令：compare
    options.add_options("Compare")("compare", "Compare multiple backends")(
        "compare-backends",
        ...
        cxxopts::value<uint32_t>()->default_value("10000"))(
        "compare-save", "Save comparison frames", cxxopts::value<bool>()->default_value("false"))(
        "compare-output", "Comparison output directory", cxxopts::value<std::string>());
    ...
    options.add_options("Positional")("command",
                                      "Command to execute (list, capture, benchmark, compare)",
...
[[nodiscard]] BackendType StringToBackendType(std::string_view str)
{
    ...
    if (lower == "adb")
        return BackendType::Adb;
    if (lower == "maa")
        return BackendType::Maa;
    if (lower == "win32")
